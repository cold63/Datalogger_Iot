...
// --- SD Card 配置 (參考 example/w5500_sd_card.ino) ---
#include "SPI.h"
#include "SdFat.h"
...
int sd_record_count = 0;
const int SD_MAX_RECORDS = 10;
...
void setup() {
    ...
    const int W5500_RESET_PIN = 10; 
    pinMode(W5500_RESET_PIN, OUTPUT);
    digitalWrite(W5500_RESET_PIN, LOW);
    delay(50); // 拉低 50ms 確保徹底重置
    digitalWrite(W5500_RESET_PIN, HIGH);
    delay(200); // 等待 W5500 內部 PLL 穩定
    
    //Serial.println("\n--- W5500 Hard Reset Done ---");
    ...
    if (sd.begin(SD_CONFIG)) {
        ...
        // 如果需要，可以在此清空舊檔案或寫入 Header
        if (logFile.open("mqtt_log.txt", O_WRITE | O_CREAT | O_TRUNC)) {
            logFile.println("RTL8720D MQTT Log Start");
            logFile.close();
        }
    } else {
        Serial.println("[SD] SD Card failed to initialize!");
    }
}
...
void loop() {
    ...
    switch (sys_state) {
        case STATE_INIT_NET:
            Serial.println("[NET] Initializing Network Hardware...");
        ...
        case STATE_DHCP:
            {
                ...
                if (ret == DHCP_IP_ASSIGN || ret == DHCP_IP_CHANGED || ret == DHCP_IP_LEASED) {
                    // 取得分配到的網路參數
        ...
        case STATE_MQTT_IDLE:
            ...
            if ((uint32_t)millis() - last_pub_time >= 10000) {
                ...
                // --- SD Card 紀錄 (連寫 100 筆) ---
                if (sd_initialized && sd_record_count < SD_MAX_RECORDS) {
                    if (logFile.open("mqtt_log.txt", O_WRITE | O_CREAT | O_AT_END)) {
                        sd_record_count++;
                        logFile.print("Record #"); logFile.print(sd_record_count);
                        logFile.print(": "); logFile.println(payload);
                        logFile.close();
                        Serial.print("[SD] Recorded #"); Serial.println(sd_record_count);
                        if (sd_record_count == SD_MAX_RECORDS) {
                            Serial.println("[SD] Reached 100 records. Logging stopped.");
                        }
                    } else {
