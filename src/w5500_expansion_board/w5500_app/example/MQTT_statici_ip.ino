/**
 * @file MQTT_static_ip.ino
 * @brief 使用 RTL8720D + W5500 實作 MQTT 發布與訂閱範例
 * 包含  IP、DNS 解析 Broker 網址、以及狀態機連線機制。
 */
...
wiz_NetInfo g_net_info = {
    ...
    .ip  = {192, 168, 1, 150},      // <-- 請在此修改為您的固定 IP
    .sn  = {255, 255, 255, 0},      // 子網路遮罩
    .gw  = {192, 168, 1, 1},       // 預設閘道 (Gateway)
    .dns = {8, 8, 8, 8},           // DNS 伺服器
    .dhcp = NETINFO_STATIC         // 設定為靜態 IP 模式
};

// --- 系統狀態機枚舉 ---
enum SystemState {
    STATE_INIT_NET,     // 初始化網路卡
    STATE_DHCP,         // 取得動態 IP (固定 IP 模式會跳過此階段)
